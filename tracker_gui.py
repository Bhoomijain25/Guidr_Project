...
import socket
import threading
from PyQt5.QtWidgets import QApplication, QWidget, QVBoxLayout, QHBoxLayout, QLabel, QListWidget, QPushButton, QListWidgetItem, QPlainTextEdit,QCheckBox, QScrollArea
...
def refine(caption):
    ...
    prompt= f"short description into something more actionable for blind person in one line : {caption}"
    # Generate response using the caption in the prompt
...
mobile_ip = 'http://192.168.126.9:8080/video'
...
class VideoWidget(QWidget):
    def __init__(self):
        ...
        super().__init__()
        self.action = 1
        self.last_navigation_time = 0  # Initialize timestamp for navigation calls

        # self.env_description_timer = QTimer()
        # self.env_description_timer.timeout.connect(self.describe_environment)
        # self.env_description_timer.start(15000)  
    ...
    def toggle_navigation(self):
        ...
        if self.navigation_started:
            ...
            self.navigation_timer.timeout.connect(self.start_navigation)
            self.navigation_timer.start(5000) 
    ...
    def infer_face(self):
        if self.frame is not None:
            ...
            print(" infer face : " + inference_result)
    ...
    def describe_environment(self):
        if self.frame is not None:
            ...
            # Display the caption in the text box
            # geminitxt = refine(caption_text)
            self.caption_textbox.setPlainText(caption_text)
            print("blip working: " + caption_text)
            print("-----------------------------------------------------------------")
            # print("genAi text: " + geminitxt)
            # print("-----------------------------------------------------------------")
